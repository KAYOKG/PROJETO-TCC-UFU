...
router.get("/summary", async (_req: Request, res: Response) => {
  try {
    ...
    const userRisks = queryToObjects(
      db,
      `
    ...
    const alertsByType = queryToObjects(
      db,
      `
    ...
    const recentAlerts = queryToObjects(
      db,
      "SELECT * FROM risk_alerts ORDER BY created_at DESC LIMIT 20",
    );

    const mlVsRules = queryToObjects(
      db,
      `
