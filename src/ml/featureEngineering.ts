...
import { SystemLog, FeatureVector, FeatureStats, FEATURE_NAMES, NUM_FEATURES } from '../types';
...
function classifyAction(action: string): string {
  ...
  if (lower.includes('cadastro') || lower.includes('criação')) return 'create';
  if (lower.includes('visualiz') || lower.includes('consulta')) return 'read';
  if (lower.includes('edição') || lower.includes('atualização') || lower.includes('status')) return 'update';
  if (lower.includes('exclusão') || lower.includes('remov')) return 'delete';
  if (lower.includes('login') || lower.includes('autenticação')) return 'login';
  if (lower.includes('configuração') || lower.includes('inatividade')) return 'config';
  return 'read';
}

function classifyModule(module: string): string {
...
export function extractFeaturesFromLog(
  ...
  const sessionStart = log.session.startTime instanceof Date
  ...
  const modulesAccessed = new Set(userLogs.map(l => classifyModule(l.origin.module)));
  ...
  if (userLogs.length > 1) {
    const sorted = userLogs
      .map(l => (l.timestamp instanceof Date ? l.timestamp : new Date(l.timestamp)).getTime())
      .sort((a, b) => a - b);
  ...
  const burstScore = userLogs.filter(l =>
  ...
  return {
    ...
    resultEncoded, sessionDurationMinutes,
    actionFrequency, actionVariety, actionSequenceEntropy,
    moduleAccessCount, sensitiveDataAccessCount, errorRate,
    avgTimeBetweenActions, burstScore, networkLatency,
    geoDistanceFromUsual, ipChangeFlag, loginAttempts,
    inactivitySeconds, isNewDevice,
  };
}
...
export function normalizeVector(vector: number[], stats: FeatureStats[]): number[] {
